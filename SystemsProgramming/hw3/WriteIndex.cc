...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  // STEP 1.
  // Write the memindex.
  int memindex_bytes = WriteMemIndex(f, mi, cur_pos);
  if (memindex_bytes < 0) {
    fclose(f);
    unlink(file_name);  // delete the file
    return kFailedWrite;
  }
  cur_pos += memindex_bytes;
  ...
  int check =  WriteHeader(f, dt_bytes, memindex_bytes);
  if (check < 0) {
    fclose(f);
    unlink(file_name);  // delete the file
    return kFailedWrite;
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  CRC32 crc;
  // We start reading after the IndexFileHeader
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    IndexFileOffset_t bucket_size = WriteHTBucket(f, bucket_pos, bucket, fn);
    // simplify using bucket record return value
    if (bucket_size < 0) {
      return kFailedWrite;
    }
    // offset
...
static int WriteHTBucketRecord(FILE* f,
  ...
  BucketRecord temp(num_elts, bucket_offset);
  temp.ToDiskFormat();
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    element_pos += written;  // you may need to change this logic
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
