...
public class HashTrieMap<A extends Comparable<A>, K extends BString<A>, V> extends TrieMap<A, K, V> {
    public class HashTrieNode extends TrieNode<ChainingHashTable<A, HashTrieNode>, HashTrieNode> {
        ...
        private class HashTrieNodeIterator extends SimpleIterator<Entry<A, HashTrieMap<A, K, V>.HashTrieNode>> {
            //private Iterator<AbstractMap.SimpleEntry<A,HashTrieMap<A, K, V>.HashTrieNode>> itr;

            private Iterator<Item<A,HashTrieNode>> itr;

            public HashTrieNodeIterator() {
                itr = pointers.iterator();
            }
            ...
            public AbstractMap.SimpleEntry<A, HashTrieNode> next() {
                ...
                Item<A,HashTrieNode> next = itr.next();
                AbstractMap.SimpleEntry<A, HashTrieNode> value = new AbstractMap.SimpleEntry<>(next.key, next.value);
                return value;
            }
        }
    ...
    public V insert(K key, V value) {
        ...
        for(int i = 0; i < key.size(); i++) {
            ...
            //represents the key
            if (temp.pointers.find(index) == null) {
                //!temp.pointers.containsKey(index)
                temp.pointers.insert(index,new HashTrieNode());
                //temp.pointers.put(index,new HashTrieNode());
            }
            temp = temp.pointers.find(index);
    ...
    private void deleteHelper(Iterator<A> itr, HashTrieNode temp) {
        ...
        if (!itr.hasNext()) {
            ...
            if (temp.pointers.find(index) != null) {
                ...
                deleteHelper(itr,temp.pointers.find(index));
