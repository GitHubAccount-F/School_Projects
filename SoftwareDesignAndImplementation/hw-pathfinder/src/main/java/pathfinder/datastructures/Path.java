...
/**
 * This represents an immutable path between two locations, particularly
 ...
 * <p>@param T1 represents the type of the path this class is using. If this is mutable, behavior is unspecified if
 *  values of this type that are passed to this class are changed</p>
 */
public class Path<T1> implements Iterable<Path<T1>.Segment> {

    // AF(this) =
    //      first place in the path => start
    //      each "step" along the path between locations => elements of list path, where
    //          path.get(0) is the first step from the starting location to an intermediate location, and
    //          path.get(path.size() - 1) is the final step from an intermediate location to the end
    ...
    /**
     * The location at the beginning of this path.
     */
    private T1 start;
     ...
     */
    private List<Path<T1>.Segment> path;

    /**
     * Creates a new, empty path containing a start. Essentially this represents a path
     * from the start to itself with a total cost of "0".
     *
     * @param start The starting location of the path.
     */
    public Path(T1 start) {
        this.start = start;
        this.cost = 0;
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last location
     ...
     */
    public Path<T1> extend(T1 newEnd, double segmentCost) {
        checkRep();
        //
        Path<T1> extendedPath = new Path<>(start);
        extendedPath.path.addAll(this.path);
        extendedPath.path.add(new Path<T1>.Segment(this.getEnd(), newEnd, segmentCost));
    ...
    /**
     * @return The location at the end of this path, which may be the start if this path
     ...
     * location and ending at the end. In the case that this path represents a path between
     ...
     */
    @Override
    public Iterator<Path<T1>.Segment> iterator() {
        // Create a wrapping iterator to guarantee exceptional behavior on Iterator#remove.
        return new Iterator<Path<T1>.Segment>() {

            private Iterator<Path<T1>.Segment> backingIterator = path.iterator();
            ...
            @Override
            public Path<T1>.Segment next() { return backingIterator.next(); }
    ...
    private void checkRep() {
        ...
        assert start != null;
        assert path != null;
        for(Path<T1>.Segment segment : path) {
            assert segment != null;
        }
    }
     ...
     * both paths are empty, they are only equal if their starting location is equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between locations.
     * Segments are immutable parts of a larger path that cannot be instantiated directly, and
     * are created as part of larger paths by calling Path#extend(location, double).
     */
    public class Segment {
         ...
         *
         * @param start The starting position of this segment.
         * @param end   The ending location of this segment.
         * @param cost  The cost of travelling this segment.
         * @throws NullPointerException     if either positions is null.
         * @throws IllegalArgumentException if cost is infinite or NaN
         */
        private Segment(T1 start, T1 end, double cost) {
            if(start == null || end == null) {
                throw new NullPointerException("Segments cannot have null locations.");
            }
        ...
        /**
         * @return The beginning location of this segment.
         */
        public T1 getStart() {
            // Note: Since locations are immutable, this isn't rep exposure.
            return this.start;
        }

        /**
         * @return The ending location of this segment.
         */
        public T1 getEnd() {
            return this.end;
        }
